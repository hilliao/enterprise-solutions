...
@RestController
public class CreateNotebooks {
    private static final Tracer tracer = Tracing.getTracer();
    private static final String template = "Slept %s seconds;";
    private final AtomicLong counter = new AtomicLong();
    public static final String logName = "create-notebooks";
    private static final Logging logging = LoggingOptions.getDefaultInstance().getService();

    @GetMapping("/sleep")
    public SleepTracked sleepAndLog(@RequestParam(value = "seconds", defaultValue = "1") String sec)
            throws InterruptedException {
        // Samplers.alwaysSample() is important to write trace; otherwise 1 out of 1000 requests is traced
    ...
    @PostMapping(path = "/notebooks", produces = org.springframework.http.MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity createAINotebooks(@RequestHeader(name = "Authorization", required = true) String Bearer,
                                            @RequestBody NotebookSpec req) throws IOException {
        Response r;

        try (Scope ss = tracer.spanBuilder("CreateAINotebook")
                .setSampler(Samplers.alwaysSample()).startScopedSpan()) {
            r = this.createAIPlatformNotebooks(
                    Bearer,
                    req.getProjectId(),
                    req.getInstanceName(),
                    req.getVPCProjectId(),
    ...
    public Response createAIPlatformNotebooks(
        ...
        String jsonContent = "{\"network\":\"projects/" + VPCProjectId + "/global/networks/" + VPCName +
                "\",\"subnet\":\"projects/" + VPCProjectId + "/regions/" + region + "/subnetworks/" + subnetName +
                "\",\"noProxyAccess\":false,\"installGpuDriver\":" + installGpuDriver + ",\"machineType\":\"" +
                machineType + "\",\"metadata\":{\"framework\":\"" + framework +
                "\"},\"bootDiskType\":\"DISK_TYPE_UNSPECIFIED\",\"bootDiskSizeGb\":\"100\",\"noPublicIp\":" +
                !publicIp + ",\"serviceAccount\":\"" + serviceAccount +
                "\",\"vmImage\":{\"project\":\"deeplearning-platform-release\",\"imageFamily\":\"" +
                imageFamily + "\"}}";

        LogEntry logDebug = LogEntry.newBuilder(StringPayload.of(
                "request JSON: " + jsonContent))
                .setSeverity(Severity.DEBUG)
                .setLogName(logName)
                .setResource(MonitoredResource.newBuilder("global").build())
                .build();
        ...
        okhttp3.RequestBody body = okhttp3.RequestBody.create(jsonContent, mediaType);
        String url = "https://notebooks.googleapis.com/v1beta1/projects/" + projectId + "/locations/" + zone +
