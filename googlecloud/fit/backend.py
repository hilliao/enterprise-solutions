...
DATE_FORMAT = '%Y-%m-%d'
ONE_DAY_MS = 86400000
STEPS_DATASOURCE = "derived:com.google.step_count.delta:" \
                   "com.google.android.gms:estimated_steps"
CALORIES_DATASOURCE = 'derived:com.google.calories.expended:' \
                      'com.google.android.gms:merge_calories_expended'
ACTIVITY_DATASOURCE = "derived:com.google.activity.segment:" \
                      "com.google.android.gms:merge_activity_segments"
HEART_RATE_DATASOURCE = 'derived:com.google.heart_rate.bpm:' \
...
if 'CLIENT_SECRET' in os.environ:
    client_secret_filename = os.environ['CLIENT_SECRET']
else:
    client_secret_filename = 'gs://next19fitness/app/client_secret.apps.googleusercontent.com.json'
...
logger_name = config.get('app_config', 'logger_name')
BQ_dataset_google_fit = config.get('bigquery_config', 'dataset_google_fit')
BQ_table_heartrate = config.get('bigquery_config', 'table_heartrate')
BQ_table_activities = config.get('bigquery_config', 'table_activities')
BQ_table_segments = config.get('bigquery_config', 'table_segments')
BQ_table_steps = config.get('bigquery_config', 'table_steps')
BQ_table_calories = config.get('bigquery_config', 'table_calories')
backend_config = """ \
    Backend Configuration: DEFAULT_TIMEZONE = {DEFAULT_TIMEZONE} \
    | DEFAULT_BUCKET = {DEFAULT_BUCKET} \
    | DATASTORE_KIND = {DATASTORE_KIND} \
    | GCP_project = {GCP_project} \
    | BQ_dataset = {BQ_dataset} \
    | BQ_table_heartrate = {BQ_table_heartrate} \
    | BQ_table_activities = {BQ_table_activities} \
    | BQ_table_segments = {BQ_table_segments} \
    | BQ_table_steps = {BQ_table_steps} \
    | BQ_table_calories = {BQ_table_calories} \
    """.format(DEFAULT_TIMEZONE=DEFAULT_TIMEZONE, DEFAULT_BUCKET=DEFAULT_BUCKET, DATASTORE_KIND=DATASTORE_KIND,
               GCP_project=GCP_project,
               BQ_dataset=BQ_dataset_google_fit,
               BQ_table_heartrate=BQ_table_heartrate,
               BQ_table_activities=BQ_table_activities,
               BQ_table_segments=BQ_table_segments,
               BQ_table_steps=BQ_table_steps,
...
def get_client_secret_filename():
    local_client_secret_filename = 'from_cloud_storage_client_secret.json'

    if os.path.isfile(local_client_secret_filename):
        return local_client_secret_filename
    elif client_secret_filename.startswith('gs://'):
        storage_client = storage.Client()
        bucket = storage_client.get_bucket(DEFAULT_BUCKET)
        len_ignore = 6 + len(DEFAULT_BUCKET)
        blob = bucket.blob(client_secret_filename[len_ignore:])
        blob.download_to_filename(local_client_secret_filename)
        return local_client_secret_filename
...
def get_daily_calories(http_auth, start_year, start_month, start_day,
                       end_time_millis, local_timezone=DEFAULT_TIMEZONE):
    """
    ...
    local_0_hour = pytz.timezone(
        local_timezone).localize(datetime(start_year, start_month, start_day))
    start_time_millis = int((local_0_hour - epoch0).total_seconds() * 1000)
    fit_service = build('fitness', 'v1', http=http_auth)
    daily_calories = {}

    calory_data = get_aggregate(fit_service, start_time_millis, end_time_millis,
                                CALORIES_DATASOURCE)
    for daily_calory_data in calory_data['bucket']:
        # use local date as the key
        local_date = datetime.fromtimestamp(
            int(daily_calory_data['startTimeMillis']) / 1000,
        ...
        if data_point:
            ...
            daily_calories[local_date_str] = {'calories': calories,
                                              'originDataSourceId': data_source_id}

    return daily_calories


def get_daily_steps(http_auth, start_year, start_month, start_day,
                    end_time_millis, local_timezone=DEFAULT_TIMEZONE
                    ):
    """
    ...
    """
    # calculate the timestamp in local time to query Google fitness API
    local_0_hour = pytz.timezone(local_timezone).localize(
        datetime(start_year, start_month, start_day))
    start_time_millis = int((local_0_hour - epoch0).total_seconds() * 1000)
    fit_service = build('fitness', 'v1', http=http_auth)
    steps = {}

    steps_data = get_aggregate(fit_service, start_time_millis, end_time_millis,
                               STEPS_DATASOURCE)
...
def get_daily_activities(http_auth, start_year, start_month, start_day, end_time_millis,
    ...
    for daily_activity in activityData['bucket']:
        ...
        get_logger().log_text('calling Google Fitness API to get activity segment from dataSetId {}'.format(
            activity_datasetId), severity='INFO')
        activity_dataset = fit_service.users().dataSources().datasets().get(
            userId="me", dataSourceId=ACTIVITY_DATASOURCE,
...
def calc_n_days_ago(past_n_days,
                    local_timezone=pytz.timezone(DEFAULT_TIMEZONE)):
    """
    ...
    :param local_timezone:
        timezone such as US/Pacific, one of the pytz.all_timezones
    ...
    n_days_ago_local_0_hour = local_timezone.localize(
        datetime(n_days_ago_local.year, n_days_ago_local.month,
                 n_days_ago_local.day))
    n_days_ago_local_0_hour_millis = (n_days_ago_local_0_hour
                                      - datetime(1970, 1, 1, tzinfo=pytz.utc)).total_seconds() * 1000
    return int(n_days_ago_local_0_hour_millis)


def get_and_insert_heart_rate(http_auth, username, start_year, start_month,
                              start_day, end_time_millis,
                              local_timezone=DEFAULT_TIMEZONE):
    """
    ...
    """
    # calculate the timestamp in local time to query Google fitness API
    local_0_hour = pytz.timezone(local_timezone).localize(datetime(start_year,
                                                                   start_month, start_day))
    start_time_millis = int((local_0_hour - epoch0).total_seconds() * 1000)
    fit_service = build('fitness', 'v1', http=http_auth)
    ...
    heartrate_data = get_aggregate(fit_service, start_time_millis,
                                   end_time_millis, HEART_RATE_DATASOURCE)
    bigquery_client = bigquery.Client()
    inserted_count = 0
    rows_to_insert = []
    query = """
        SELECT recordedTimeNanos 
        FROM `{}.{}.{}` 
        WHERE username = '{}' AND recordedLocalDate >= '{}'
        """.format(
        GCP_project, BQ_dataset_google_fit, BQ_table_heartrate, username,
        "{}-{}-{}".format(start_year, start_month, start_day))
    ...
    for daily_item in heartrate_data['bucket']:
        incoming_day_localized = datetime.fromtimestamp(
            int(daily_item['startTimeMillis']) / 1000,
            tz=pytz.timezone(local_timezone))
        incoming_day_localized_str = incoming_day_localized.strftime(
            DATE_FORMAT)
        ...
        if data_point:
            ...
            get_logger().log_text('calling Google Fitness API to get heart rate from dataSetId {} for user {}'.format(
                heart_datasetId, username), severity='INFO')
            heart_dataset = fit_service.users().dataSources().datasets().get(
                userId="me", dataSourceId=HEART_RATE_DATASOURCE,
            ...
            for bpm_data_point in data_point_list:
                if int(bpm_data_point['endTimeNanos']) not in existing_rows:
                    ...
                    rows_to_insert.append(
                        (username, int(bpm_data_point['endTimeNanos']),
                         incoming_day_localized_str,
                         int(bpm_data_point['value'][0]['fpVal'])))

    if rows_to_insert:
        dataset_ref = bigquery_client.dataset(BQ_dataset_google_fit)
        table_ref = dataset_ref.table(BQ_table_heartrate)
        table = bigquery_client.get_table(table_ref)
        # BigQuery API request
        errors = bigquery_client.insert_rows(table, rows_to_insert)
        if errors:
            get_logger().log_text(str(errors), severity='ERROR')
...
def insert_steps(username, steps, local_timezone=DEFAULT_TIMEZONE):
    ...
    bigquery_client = bigquery.Client()
    dataset_ref = bigquery_client.dataset(BQ_dataset_google_fit)
    table_steps_ref = dataset_ref.table(BQ_table_steps)
    table_steps = bigquery_client.get_table(table_steps_ref)

    # check existing rows by local date
    query = """
        SELECT DISTINCT recordedLocalDate 
        FROM `{}.{}.{}` 
        WHERE username = '{}' 
        ORDER BY recordedLocalDate DESC
        """.format(GCP_project, BQ_dataset_google_fit, BQ_table_steps, username)
    query_job = bigquery_client.query(query)
    existing_step_dates = [
        row['recordedLocalDate'] for row in query_job.result()]
    rows_to_insert = []
    now_utc = datetime.now(pytz.timezone('UTC'))
    ...
    for localDate, value in steps.iteritems():
        ...
        if incoming_steps_date not in existing_step_dates:
            rows_to_insert.append(
                (username, localDate, value['steps'],
                 value['originDataSourceId'])
            )
...
def insert_calories(username, calories, local_timezone=DEFAULT_TIMEZONE):
    ...
    bigquery_client = bigquery.Client()
    dataset_ref = bigquery_client.dataset(BQ_dataset_google_fit)
    table_calories_ref = dataset_ref.table(BQ_table_calories)
    ...
    query = """
        ...
        WHERE username = '{}' 
        ORDER BY recordedLocalDate DESC
        """.format(GCP_project, BQ_dataset_google_fit, BQ_table_calories, username)
    query_job = bigquery_client.query(query)
    existing_calories_dates = [
        row['recordedLocalDate'] for row in query_job.result()]
    rows_to_insert = []
    now_utc = datetime.now(pytz.timezone('UTC'))
    now_local = now_utc.astimezone(pytz.timezone(local_timezone))
    ...
    for localDate, value in calories.iteritems():
        incoming_calories_date = datetime.strptime(
            localDate, DATE_FORMAT).date()
        ...
        if incoming_calories_date not in existing_calories_dates:
            rows_to_insert.append((username, localDate, value['calories'],
                                   value['originDataSourceId']))
...
def insert_activities(username, activities, local_timezone=DEFAULT_TIMEZONE):
    ...
    bigquery_client = bigquery.Client()
    dataset_ref = bigquery_client.dataset(BQ_dataset_google_fit)
    table_activities_ref = dataset_ref.table(BQ_table_activities)
    table_activities = bigquery_client.get_table(table_activities_ref)
    table_segments_ref = dataset_ref.table(BQ_table_segments)
    table_segments = bigquery_client.get_table(table_segments_ref)

    # check existing rows by local date in the table
    query = """
        SELECT recordedLocalDate, COUNT(activity_type) AS activity_type_count 
        FROM `{}.{}.{}` 
        WHERE username = '{}' 
        GROUP BY recordedLocalDate 
        ORDER BY recordedLocalDate DESC
        """.format(
        GCP_project, BQ_dataset_google_fit, BQ_table_activities, username)
    query_job = bigquery_client.query(query)
    ...
    for localDate, value in activities.iteritems():
        incoming_activity_date = datetime.strptime(localDate,
                                                   DATE_FORMAT).date()
        ...
        # if incoming activity's date not found in the existing activities
        #  table, insert incoming activities
        if incoming_activity_date not in existing_activity_dates:
            for daily_activity in value['daily_activities']:
                activity_rows_to_insert.append(
                    (username, localDate, daily_activity['activity_type'],
                     daily_activity['seconds'], daily_activity['segments']))
            ...
            for point in value['activity_dataset']['point']:
                ...
                segment_rows_to_insert.append(
                    (username, localDate, activity_type, point['startTimeNanos']
                     , point['endTimeNanos'], point['originDataSourceId']))
    if activity_rows_to_insert:
        # BigQuery API request
        errors = bigquery_client.insert_rows(table_activities,
                                             activity_rows_to_insert)
        if errors:
            get_logger().log_text(str(errors), severity='ERROR')
            raise Exception(str(errors))
    if segment_rows_to_insert:
        # BigQuery API request
        errors = bigquery_client.insert_rows(table_segments,
                                             segment_rows_to_insert)
        if errors:
            get_logger().log_text(str(errors), severity='ERROR')
            raise Exception(str(errors))
...
class UserDataFlow:
    def __init__(self, username, http_auth, start_year, start_month, start_day,
                 end_time_millis, local_timezone):
        ...
        user_data_flow_configs = """
            UserDataFlow instantiated with configs: 
                username={username}, 
                http_auth={http_auth}, 
                ...
                """.format(username=self.username,
                           ...
                           start_year=self.start_day,
                           start_month=start_month,
                           start_day=start_day,
                           end_time_millis=self.end_time_millis,
                           local_timezone=self.local_timezone)

        get_logger().log_text(backend_config, severity='INFO')
        get_logger().log_text(user_data_flow_configs, severity='INFO')

    def get_steps(self):
        self.steps = get_daily_steps(
            self.http_auth, self.start_year, self.start_month, self.start_day, self.end_time_millis,
            self.local_timezone)
        return self.steps

    def post_steps(self):
        if self.steps is not None:
            self.insert_steps_result = insert_steps(
                self.username, self.steps, self.local_timezone)
            return self.insert_steps_result
        else:
            get_logger().log_text("no self.steps to insert to BigQuery", severity='ERROR')
            raise RuntimeError('no self.steps to insert to BigQuery')

    def get_calories(self):
        self.calories = get_daily_calories(
            self.http_auth, self.start_year, self.start_month, self.start_day,
            self.end_time_millis, self.local_timezone)
        return self.calories

    def post_calories(self):
        if self.calories is not None:
            self.insert_calories_result = insert_calories(
                self.username, self.calories, self.local_timezone)
            return self.insert_calories_result
        else:
            get_logger().log_text("no self.calories to insert to BigQuery", severity='ERROR')
            raise RuntimeError('no self.calories to insert to BigQuery')

    def get_and_post_heart_rate(self):
        self.insert_heart_rate_result = get_and_insert_heart_rate(
            self.http_auth, self.username, self.start_year, self.start_month,
            self.start_day, self.end_time_millis, self.local_timezone)
        return self.insert_heart_rate_result

    def get_activities(self):
        self.activities = get_daily_activities(
            self.http_auth, self.start_year, self.start_month, self.start_day,
            self.end_time_millis, self.local_timezone)
    ...
    def post_activities(self):
        if self.activities is not None:
            self.insert_activities_result = insert_activities(
                self.username, self.activities, self.local_timezone)
            return self.insert_activities_result
        else:
            get_logger().log_text("no self.activities to insert to BigQuery", severity='ERROR')
